...
class UserAccountTest extends DuskTestCase
{
	...
	public function testAccountDropdownIsWorking()
	{
		$this->browse(function (Browser $browser) {
			$browser->loginAs(app('oxygen')::makeUserModel()::find(2))
				->visit(new Dashboard())
	...
	public function testEditingPassword()
	{
		$this->browse(function (Browser $browser) {
			$browser->loginAs(app('oxygen')::makeUserModel()::find(2))
				...
				->click('#accountEditPasswordForm button[type="submit"]')
	...
	{
		$this->browse(function (Browser $browser) {
			$user = app('oxygen')::makeUserModel()::find(2);
			$name = $user->name;
			...
			$browser->loginAs($user)
				...
				->click('#accountProfileForm button[type="submit"]')
	...
	public function testEditingEmail()
	{
		$this->browse(function (Browser $browser) {
			...
			$browser->loginAs($user)
				...
				->clear('#password')->type('#password', '12345678')
				->click('#accountEditEmailForm button[type="submit"]')
